...
test.describe("Authentication", () => {
  ...
  test.describe("User is authenticated", () => {
    test("should navigate to /onboarding when authenticated and no profile", async ({ page, context }) => {
    ...
    test("should not redirect when authenticated and profile found", async ({ page, context }) => {
      const authState = await setupAuthState({
        userId: "test-user-id",
        email: "test@example.com"
      });
