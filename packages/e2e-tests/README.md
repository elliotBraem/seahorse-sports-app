...
In order to represent different roles via an authenticated session, you can mock logins through [storage states](https://playwright.dev/docs/auth)..
...
test.describe("User is logged in", () => {
  ...
  test.use({
    storageState: "playwright-tests/storage-states/user-connected.json",
  });
